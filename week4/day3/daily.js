...
function getCarHonda(carInventory) {
    const getFirstHonda = carInventory.filter((item,i)=>{
        return item.car_make === 'Honda'
    })
    return getFirstHonda[0]
}

function returnString(car){
    console.log(`This is a ${car.car_make} ${car.car_model} from ${car.car_year}`)
}
returnString(getCarHonda(inventory))
...
const sorted = carInventory.sort((a,b)=>{
